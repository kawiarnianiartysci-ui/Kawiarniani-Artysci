...
const CLIENT_TERMS_SECTIONS = [
  { title:"§ 1. Postanowienia ogólne", items: [
    ...
    "2. Usługodawcą i podmiotem prowadzącym Platformę jest Joanna Cybulska prowadząca działalność w formie działalności nierejestrowanej w rozumieniu art. 5 ustawy z dnia 6 marca 2018 r. – Prawo przedsiębiorców, adresem e-mail: kawiarnianiartysci@gmail.com (dalej: „Usługodawca”).",
    "3. Regulamin został sporządzony w oparciu o przepisy ustawy z dnia 18 lipca 2002 r. o świadczeniu usług drogą elektroniczną, ustawy z dnia 30 maja 2014 r. o prawach konsumenta oraz Kodeksu cywilnego.",
    "4. Regulamin jest udostępniony nieodpłatnie na stronie Platformy w formie umożliwiającej jego pobranie, zapisanie i wydrukowanie.",
  ]},
  { title:"§ 2. Definicje", items: [
    "Platforma – serwis internetowy dostępny pod adresem kawiarnianiartysci.pl.",
    "Klient / Konsument – osoba fizyczna korzystająca z Platformy w celu zorganizowania wydarzenia grupowego.",
    "Partner – restauracja lub artysta prowadzący warsztaty, prezentujący swoją ofertę na Platformie.",
    "Artysta – Partner prowadzący Warsztat w ramach Wydarzenia.",
    "Restauracja – Partner świadczący usługę gastronomiczną w ramach Wydarzenia.",
    ...
    "Zapytanie – wiadomość przesłana przez Klienta za pośrednictwem formularza na Platformie, zawierająca wybrane parametry Wydarzenia.",
    "Wymagania techniczne: korzystanie z Platformy wymaga posiadania urządzenia z dostępem do sieci Internet, przeglądarki internetowej obsługującej standardowe technologie webowe oraz aktywnego adresu e-mail.",
  ]},
  ...
  { title:"§ 8. Zasady organizacji Eventów i Urodzin dla Dzieci (Małoletnich)", items: [
    ...
    "4. Klient odpowiada za zapewnienie liczby Opiekunów adekwatnej do liczby i wieku Małoletnich uczestników oraz charakteru Warsztatu.",
    "5. Artysta odpowiada wyłącznie za należyte, zgodne z programem i zasadami bezpieczeństwa przeprowadzenie Warsztatu lub animacji. Artysta nie sprawuje opieki nad Małoletnimi i nie ponosi odpowiedzialności za ich zachowanie poza zakresem prowadzonych przez siebie zajęć ani za szkody wyrządzone przez Małoletnich sobie nawzajem, Restauracji, Artyście lub osobom trzecim.",
    "6. Restauracja odpowiada za jakość i bezpieczeństwo serwowanych potraw oraz stan techniczny udostępnianego lokalu. Restauracja nie sprawuje opieki nad Małoletnimi i nie ponosi odpowiedzialności za szkody wyrządzone przez Małoletnich w lokalu lub w mieniu osób trzecich.",
    "7. Odpowiedzialność za szkody wyrządzone przez Małoletniego uczestnika Wydarzenia ponosi Opiekun sprawujący nad nim nadzór w chwili wyrządzenia szkody, zgodnie z art. 426–427 Kodeksu cywilnego, a w zakresie, w jakim obowiązek zapewnienia odpowiedniej liczby Opiekunów leżał po stronie Klienta – także Klient.",
    "8. Zgłaszając Event Dziecięcy, Klient oświadcza, że posiada zgody rodziców/opiekunów prawnych wszystkich Małoletnich uczestników na ich udział w Wydarzeniu oraz, w zakresie uzgodnionym z Restauracją i/lub Artystą, na wykonywanie i wykorzystywanie wizerunku Małoletnich utrwalonego podczas Wydarzenia (art. 81 ustawy o prawie autorskim i prawach pokrewnych).",
...
function ClientTermsModal({ onClose }) {
  const section = { marginBottom: 18 };
  const h = { fontFamily:"'Montserrat', system-ui, sans-serif", fontSize:15, fontWeight:600, color:C.text, marginBottom:6 };
  const p = { fontSize:13, color:C.muted, lineHeight:1.7, margin:"0 0 6px" };
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.65)", zIndex:600, overflowY:"auto", padding:"20px 12px", display:"flex", alignItems:"flex-start", justifyContent:"center" }}>
      <div style={{ background:"#FFF", borderRadius:16, maxWidth:640, width:"100%", padding:"32px 28px", marginBottom:40 }}>
        ...
        <p style={{ ...p, marginBottom:22, fontStyle:"italic" }}>Regulamin świadczenia usług drogą elektroniczną — zasady korzystania z Platformy przez Klientów. Obowiązuje od 22.08.2026.</p>

        {CLIENT_TERMS_SECTIONS.map(s => (
          <div key={s.title} style={section}>
            <div style={h}>{s.title}</div>
            {s.items.map((it, i) => <p key={i} style={p}>{it}</p>)}
          </div>
  ...
  return (
    <>
      <div style={{ textAlign:"center", padding:"20px 16px", fontSize:11, color:C.muted, borderTop:`1px solid ${C.border}` }}>
        ...
        {" "}·{" "}
        <button onClick={() => setShowTerms(true)} style={{ background:"none", border:"none", color:C.muted, textDecoration:"underline", cursor:"pointer", fontSize:11, padding:0 }}>
          Regulamin
        </button>
      </div>
      {showPrivacy && <PrivacyPolicyModal onClose={() => setShowPrivacy(false)} />}
      {showTerms && <ClientTermsModal onClose={() => setShowTerms(false)} />}
    </>
  );
}
      ...
      {empty ? (
        <div style={{ textAlign:"center", padding:"40px 20px", background:C.card, borderRadius:14, border:`1px solid ${C.border}` }}>
          ...
          <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
            ...
            <button onClick={onFallback} style={{ padding:"12px 22px", borderRadius:999, border:`1px solid ${C.border}`, background:"#FFF", color:C.text, fontSize:13, fontWeight:600, cursor:"pointer", minHeight:44 }}>Napisz do nas</button>
          </div>
        </div>
      ) : (
...
function Step4ContactForm({ restaurant, variant, workshop, groupSize, selectedDate, onDateChange, selectedTime, onTimeChange, ppp, total, workshopOnlyTotal, onEditStep, onSubmitted, kidsMode = false, kidsCount, adultsCount, ownPlace = false, placeInfo, workshopStep = 1, placeStep = 2, requesterType, invoiceRequired }) {
  ...
  return (
    <div style={{ maxWidth:480, margin:"0 auto", padding:"24px 16px 60px" }}>
      ...
      <label style={{ display:"flex", gap:9, alignItems:"flex-start", fontSize:11, color:C.muted, lineHeight:1.5, marginBottom:6, cursor:"pointer" }}>
        <input type="checkbox" checked={termsAccepted} onChange={e => setTermsAccepted(e.target.checked)} style={{ marginTop:2, flexShrink:0, width:18, height:18 }} />
        <span>Zapoznałem/-am się i akceptuję{" "}
          <button type="button" onClick={e => { e.preventDefault(); setShowTerms(true); }} style={{ background:"none", border:"none", padding:0, color:C.primary, textDecoration:"underline", cursor:"pointer", fontSize:11 }}>
            Regulamin Platformy
          </button>.
        </span>
      </label>
      {errors.terms && <div style={{ ...errStyle, marginBottom:12 }}>{errors.terms}</div>}
                      ...
                      <div style={{ maxWidth:1160, margin:"20px auto 0", padding:"0 16px" }}>
                        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, boxShadow:"0 1px 5px rgba(0,0,0,0.07)", padding:"18px 20px", marginBottom:20 }}>
                          ...
                          <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
                            <div style={{ fontSize:11, fontWeight:600, color:C.muted }}>Wymagana faktura VAT</div>
                            <div style={{ display:"flex", gap:6 }}>
                              <button onClick={() => setInvoiceRequired(false)} style={{ padding:"5px 12px", borderRadius:999, cursor:"pointer", background: !invoiceRequired ? C.selectedBg : C.card, border:`1.5px solid ${!invoiceRequired ? C.primary : C.border}`, color:C.primary, fontWeight:500, fontSize:11, fontFamily:"'Montserrat', system-ui, sans-serif" }}>Nie</button>
                              <button onClick={() => setInvoiceRequired(true)} style={{ padding:"5px 12px", borderRadius:999, cursor:"pointer", background: invoiceRequired ? C.selectedBg : C.card, border:`1.5px solid ${invoiceRequired ? C.primary : C.border}`, color:C.primary, fontWeight:500, fontSize:11, fontFamily:"'Montserrat', system-ui, sans-serif" }}>Tak</button>
                            </div>
                          </div>
                        </div>
                      </div>
