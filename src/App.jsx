        ...
        <div style={{ padding:"24px 24px 32px" }}>
          ...
          {item.photos ? (
            ...
            <div style={{ borderRadius:10, overflow:"hidden", marginBottom:20 }}>
              <img src={item.photo} alt={item.name} loading="lazy" style={{ width:"100%", height:180, objectFit:"cover", objectPosition: item.photo.includes("workshop-painting-photo") ? "center 25%" : item.photo.includes("agata-photo") ? "center 70%" : "center", display:"block" }} />
            </div>
          ) : (
...
function WorkshopCard({ w, isSelected, onToggle, onProfile, kidsMode = false, ownPlace = false }) {
  ...
  return (
    <div className={soon ? "" : "card-h"} style={{ background: isSelected ? C.selectedBg : soon ? "#F5F4F1" : C.card, border:`2px solid ${isSelected ? C.primary : "transparent"}`, borderRadius:14, overflow:"hidden", boxShadow: isSelected ? "0 4px 16px rgba(67,42,22,0.14)" : soon ? "none" : "0 1px 5px rgba(0,0,0,0.07)", position:"relative", opacity: soon ? 0.78 : 1, display:"flex", flexDirection:"column", width:"100%" }}>
      {w.photo ? (
        <div style={{ height:140, overflow:"hidden", position:"relative" }}>
          <img src={w.photo} alt={w.name} loading="lazy" style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition: w.photo.includes("workshop-painting-photo") ? "center 25%" : w.photo.includes("agata-photo") ? "center 70%" : "center", display:"block" }} />
        </div>
      ) : w.logo ? (
